...
export const usePoolsList = ({
  ...
  const processed = useMemo((): PoolItem[] => {
    ...
    return poolsList.map((pool) => ({
      address: pool.id,
      token1: {
        image: pool.iconUrl1 === '' || pool.iconUrl1=== undefined ? getIconUrl(pool.token1) : pool.iconUrl1,
        name: pool.name1,
      },
      token2: {
        image: pool.iconUrl2 === ''||pool.iconUrl2===undefined ? getIconUrl(pool.token2) : pool.iconUrl2,
        name: pool.name2,
      },
